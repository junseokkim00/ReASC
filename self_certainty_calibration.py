...
from utils_hf import set_seed, set_xverify_evaluator, xverify_eval
import random
import os
...
if __name__ == "__main__":
    ...
    llm = LLM(model=args.model_name, tensor_parallel_size=torch.cuda.device_count())
    ...
    if args.dataset == 'gsm8k':
        ...
        # select args.sample_size samples from ds randomly
        if args.sample_size != -1:
            ds = random.sample(ds, args.sample_size)
    elif args.dataset == 'math':
        with open("./math500/train.jsonl", "r") as f:
    ...
    if args.batch_size == 1:
        for idx, inst in enumerate(tqdm(ds)):
            ...
            chat_template = [{'role': 'user', 'content': question}]
            ...
            decoded_response = decode_outputs[0]
            sc_per_tokens = sc_per_token_batch[0]
            sc_overall = sc_overall_batch[0]
            with open(f"./logs/self_certainty_samples/sample_{args.sample_size}_{args.dataset}_{args.max_new_tokens}_self_certainty_{args.model_name.replace('/', '_')}_seed_{args.seed}.jsonl", "a") as f:
                f.write(json.dumps({
                    'index': idx,
                    'question': question,
                    'answer': answer,
        ...
        for idx, inst in enumerate(tqdm(ds)):
            ...
            idxs.append(idx)
            chat_template = [{'role': 'user', 'content': question}]
            batch_chat.append(chat_template)
            cnt+=1
            if cnt < args.batch_size and idx != len(ds)-1:
                continue
            else:
                outs = generation_w_self_certainty_vllm(
                    chat=batch_chat,
                ...
                decode_outputs, _, sc_per_token_batch, sc_overall_batch = outs
                with open(f"./logs/self_certainty_samples/sample_{args.sample_size}_{args.dataset}_{args.max_new_tokens}_self_certainty_{args.model_name.replace('/', '_')}_seed_{args.seed}.jsonl", "a") as f:
                    for decoded_response, sc_per_tokens, sc_overall, question, answer, idx in zip(decode_outputs, sc_per_token_batch, sc_overall_batch, questions, answers, idxs):
                        f.write(json.dumps({
                            'index': idx,
                            'question': question,
                            'answer': answer,
